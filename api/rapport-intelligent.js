...
###PROPOSITION###
{"type":"commande","po":"<numéro de PO exact tel que dans les données>","champ":"notes"|"dateLivraisonPrevue"|"statut"|"flagFactureNonLivree","valeur":"<nouvelle valeur>","facture":"<numéro de facture exact — uniquement requis si champ=flagFactureNonLivree>","resume":"<résumé court en français de ce qui va changer>"}
###FIN###
Règles : "champ" ne peut être que "notes" (ajoute une note — n'écris que le texte à ajouter, pas les notes existantes), "dateLivraisonPrevue" (format YYYY-MM-DD), "statut" (SEULE valeur valide : "annule" — les autres statuts sont automatiques), ou "flagFactureNonLivree" (signale/désigne UNE facture précise — pas la commande entière — comme contestée par le client malgré la facturation ; valeur "true" pour signaler, "false" pour retirer ; nécessite le champ "facture" avec le numéro exact de la facture concernée, présent dans les données de la commande). "po" doit correspondre exactement à un po présent dans les données — si tu ne le trouves pas avec certitude, ne propose rien et demande une précision.
Chaque commande du contexte peut contenir un champ 'factureNonLivree' (présent seulement si au moins une facture est signalée) avec 'montantConcerne' et 'facturesConcernees' (numéros des factures signalées) — ce signalement est TOUJOURS manuel : il ne peut être posé que par une personne qui sait qu'un client conteste une livraison, jamais déduit automatiquement d'une date. Si l'utilisateur demande un état des lieux, utilise ce champ directement. Ne propose 'flagFactureNonLivree' QUE si l'utilisateur t'indique explicitement qu'un client conteste ou n'a pas reçu une livraison précise malgré une facture — jamais sur la base d'une simple absence de date de disponibilité dans les données, qui n'est pas un signal fiable dans ce métier (la marchandise est souvent expédiée et facturée simultanément, sans que la date de disponibilité soit systématiquement renseignée).
    ...
    if (match) {
      ...
      try {
        ...
        const isValidCommandeProposal =
          ...
          !(parsed.champ === "flagFactureNonLivree" && !(typeof parsed.facture === "string" && parsed.facture));
